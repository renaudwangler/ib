...

{
    ...
    foreach ($resourceGroupName in (Get-AzureRMResourceGroup).ResourceGroupName) {
        if ($resourceGroupName -like "20533E$labNumberTwoDigit*") {
             ...
             Start-Job -ScriptBlock {Remove-AzureRMResourceGroup -Name $args[1] -Force -InformationAction SilentlyContinue -AzureRmContext $args[0] } -ArgumentList (Get-AzureRmContext),$resourceGroupName -InformationAction SilentlyContinue  |Out-Null
        }
    }
    ...
    Disable-AzureRmContextAutosave|Out-Null

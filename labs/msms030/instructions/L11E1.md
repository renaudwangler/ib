...
1. In the **Pick an account** window, select **Holly Dickson** at **holly@M365xZZZZZZ.onmicrosoft.com** (where ZZZZZZ is the tenant ID provided by your lab hosting provider) and enter **ibForm@tion** as her password.
...
1. In the **Assign owners** window, select **+ Assign owners**, enter **MOD** and press **Enter** in the **Search for a name or email address** field, which will display the list of active users whose first name starts with MOD. Select **MOD Administrator** and then select **Add (1)**. When **MOD Administrator** is listed, select **Next**.
...
1. In the **Edit settings** window, enter **PNDgroup** in the **Group email address** field.  
	**Note:** To the right of the **Group email address** field is the domain field. This is different from adding users in that no other domains appear; therefore, you must leave this value as is.  
	After configuring this field, the PND Group email address should appear as: **PNDgroup@xxxUPNxxx.xxxCustomDomainxxx.xxx**  
...
1. For security purposes, you have decided to add Diego Siciliani as an additional owner of this group. In the **PND Group** pane that appears, the **General** tab is displayed by default. Select the **Membership** tab. 
1. In the **Membership** tab, in the **Owners** section, select **+ Add owners**. This displays an **Add team owners to PND Group** pane.
1. In the Search field, enter **Diego**, select **Diego Siciliani**, and then select **Add (1)**.
...
1. At the command prompt, enter the following command and then press Enter to install the Exchange Online Management Module into Windows PowerShell.  
...
1. 	At the command prompt enter the following command and then press Enter (remember to replace ZZZZZZ with the tenant ID provided by your lab hosting provider:  
...
1. At the command prompt enter the following command and then press Enter to validate that your are connected to the Microsoft 365 Compliance center:  
	```Get-DlpSensitiveInformationType -Identity "Credit Card Number"```
1.	At the command prompt enter the following command and then press Enter to create a new sensitivity label named “Secret”, set its Tooltip to “Use it for Government Contracts ONLY" and changes the text color to Red:  
